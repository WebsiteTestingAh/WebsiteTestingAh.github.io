...
async function checkStatus() {
  try {
    ...
    apiStatus.querySelector("span:last-child").textContent = data.ready
      ? "Local AI ready"
      : data.modelInstalled && !data.visualAnalysisReady
        ? "Video tools setup"
        : "Free AI setup";
    $("#setup-ai-button").hidden = data.ready;
    analysisMode.value = data.defaultAnalysisMode || "fast";
    analysisMode.querySelector('option[value="fast"]').disabled = !data.fastModelInstalled;
    analysisMode.querySelector('option[value="deep"]').disabled = !data.deepModelInstalled;
  } catch {
...
$("#setup-ai-button").addEventListener("click", () => {
  const needsVideoTools = aiStatus?.ollamaRunning && aiStatus?.modelInstalled && !aiStatus?.visualAnalysisReady;
  const message = needsVideoTools
    ? "Download the free local video tools once. GitHub does not store these large binaries, so Whisk fetches them after cloning."
  ...
  openModal(message, needsVideoTools ? "Set up free video tools" : "Set up your free local AI", aiStatus?.setupCommand || "ollama pull gemma3:4b");
});
...
function renderRecipe(recipe, analysis = null) {
  ...
  visualReport.textContent = analysis?.visual?.found
    ? `${analysis.visual.fallbackUsed ? "Attached clip fallback" : "Automatic platform video review"} used ${analysis.visual.sampledFrames} sampled frames${
        analysis.analysisMode ? ` in ${analysis.analysisMode === "deep" ? "Deep review" : "Fast local"} mode` : ""
      }.`
    : "";
