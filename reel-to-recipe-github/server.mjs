...
const fastModel = process.env.LOCAL_FAST_MODEL || "gemma3:4b";
const deepModel = process.env.LOCAL_RECIPE_MODEL || "gemma3:12b";
const defaultAnalysisMode = process.env.DEFAULT_ANALYSIS_MODE === "deep" ? "deep" : "fast";
...
const maxCachedAnalyses = 12;
const analysisCacheTtlMs = 6 * 60 * 60 * 1000;
  ...
  try {
    ...
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        "90M",
        "--format",
        "wv*[height>=360][height<=480]/wv*[height<=480]/b[height<=360]/b[height<=480]/wv*",
        "--output",
        outputTemplate,
        "--print",
...
function ollamaModelIsInstalled(models = [], modelName) {
  return models.some((installedModel) => {
    const name = installedModel.name || installedModel.model || "";
    return name === modelName || name === `${modelName}:latest` || (modelName.endsWith(":latest") && name === modelName.slice(0, -7));
  });
}

function modelForAnalysisMode(mode) {
  return mode === "deep" ? deepModel : fastModel;
}

function analysisCacheKey({ url, notes, analysisMode }) {
  return `${analysisMode}\n${url}\n${notes}`;
}
...
function getCachedAnalysis(key) {
  ...
  if (Date.now() - cached.createdAt > analysisCacheTtlMs) {
...
function setCachedAnalysis(key, payload) {
  analysisCache.set(key, { createdAt: Date.now(), payload });
  while (analysisCache.size > maxCachedAnalyses) {
...
async function getLocalAiStatus() {
  ...
  try {
    ...
    const fastModelInstalled = ollamaModelIsInstalled(payload.models, fastModel);
    const deepModelInstalled = ollamaModelIsInstalled(payload.models, deepModel);
    const model = modelForAnalysisMode(defaultAnalysisMode);
    const modelInstalled = defaultAnalysisMode === "deep" ? deepModelInstalled : fastModelInstalled;
    const visualAnalysisReady = videoDownloadHelperReady && frameExtractorReady;
    const ready = modelInstalled && visualAnalysisReady;
    return {
      ready,
      provider: "ollama",
      model,
      ...
      deepModel,
      deepModelInstalled,
      setupCommand: ready ? "" : modelInstalled ? "npm run setup:tools" : `ollama pull ${model}`,
...
function buildPrompt({ url, metadata, frameCount }) {
  return [
    ...
    "Transcript and user-note details have already been summarized into the grounded evidence report.",
  ].join("\n");
}
...
async function chatWithOllama({ format, messages, modelName, numCtx = 8192 }) {
...
async function extractEvidenceWithOllama({ url, notes, frames, metadata, modelName }) {
...
async function buildRecipeWithOllama({ url, metadata, frames, evidence, modelName }) {
  return chatWithOllama({
    modelName,
    numCtx: 4096,
    format: recipeSchema,
    messages: [
      ...
      },
      {
        role: "user",
        content: [
          buildPrompt({ url, metadata, frameCount: frames.length }),
          "",
...
function hasExplicitQuantity(value) {
  return /^(?:\d+(?:\s+\d+\/\d+|\/\d+|\.\d+)?|one|two|three|four|five|six|seven|eight|nine|ten)(?:\s+\S.*)?$/i.test(
    value.trim(),
  );
}

function sanitizeEvidence(evidence) {
  evidence.visibleIngredients = evidence.visibleIngredients.map((ingredient) => ({
    ...ingredient,
    statedAmount: hasExplicitQuantity(ingredient.statedAmount) ? ingredient.statedAmount : "",
  }));
  return evidence;
}
...
async function handleAnalyze(request, response) {
  ...
  try {
    ...
    const analysisMode = body.analysisMode === "deep" ? "deep" : "fast";
    const model = modelForAnalysisMode(analysisMode);
    ...
    const cacheKey =
      parsedUrl && !attachedFrames.length
        ? analysisCacheKey({ url: parsedUrl.href, notes: userNotes, analysisMode })
        : "";
    const cachedAnalysis = cacheKey ? getCachedAnalysis(cacheKey) : null;
    if (cachedAnalysis) {
      return sendJson(response, 200, cachedAnalysis);
    }
    const aiStatus = await getLocalAiStatus();
    const selectedModelInstalled = analysisMode === "deep" ? aiStatus.deepModelInstalled : aiStatus.fastModelInstalled;
    if (!aiStatus.ollamaRunning || !selectedModelInstalled) {
      const setupCommand = `ollama pull ${model}`;
      const error = aiStatus.ollamaRunning
        ? `The ${analysisMode} local AI is almost ready. Run "${setupCommand}" once, then try again.`
    ...
    const [{ metadata, transcript }, platformVisual] = await Promise.all([
      fetchVideoContext(parsedUrl),
      parsedUrl ? downloadAndSamplePlatformVideo(parsedUrl) : emptyVisual("No platform URL was supplied."),
    ]);
    ...
    const evidence = sanitizeEvidence(await extractEvidenceWithOllama({
      url: parsedUrl?.href || "",
      notes,
      frames,
      metadata,
    ...
    const recipeMs = Math.round(performance.now() - recipeStartedAt);
    const payload = {
      recipe,
      analysis: {
        mode: "local-ai",
    ...
    if (cacheKey) setCachedAnalysis(cacheKey, payload);
...
server.listen(port, () => {
  ...
  console.log(`Free local AI mode uses Ollama with ${fastModel} for fast review and ${deepModel} for deep review.`);
});
